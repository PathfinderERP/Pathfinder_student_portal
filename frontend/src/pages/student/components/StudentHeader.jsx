...
const StudentHeader = ({ activeTab }) => {
    ...
    return (
        <header className={`sticky top-0 z-40 px-6 py-4 rounded-[2rem] border transition-all backdrop-blur-md
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                    {/* Search */}
                    <button className={`p-3 rounded-xl transition-all hover:scale-110 active:scale-95
                        ${isDarkMode ? 'bg-white/5 hover:bg-white/10 text-slate-400' : 'bg-slate-50 hover:bg-slate-100 text-slate-600'}`}>
                        <Search size={18} strokeWidth={2.5} />
                    </button>

                    {/* Notifications */}
                    <button className={`relative p-3 rounded-xl transition-all hover:scale-110 active:scale-95
                        ${isDarkMode ? 'bg-white/5 hover:bg-white/10 text-slate-400' : 'bg-slate-50 hover:bg-slate-100 text-slate-600'}`}>
