...
const ExamEngine = () => {
    ...
    if (isSubmitted) {
        ...
        return (
            <div className="min-h-screen w-full bg-[#f8fafc] flex flex-col items-center justify-center p-3 sm:p-6 animate-in fade-in zoom-in duration-500">
                {/* VIOLATION OVERLAY - Fixed to the top of everything */}
                {showViolation && (
                    <div className="fixed inset-0 bg-white/95 backdrop-blur-[15px] z-[99999] flex items-center justify-center p-4 sm:p-8 animate-in fade-in zoom-in-95 duration-500">
                        <div className="max-w-xl w-full bg-white p-6 sm:p-12 rounded-[1.5rem] sm:rounded-[2.5rem] shadow-[0_50px_100px_rgba(0,0,0,0.1)] text-center border border-gray-100 relative overflow-hidden">
                            ...
                            <div className="w-16 h-16 sm:w-24 sm:h-24 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-6 sm:mb-8 animate-bounce transition-all">
                                <AlertCircle className="text-red-600 w-8 h-8 sm:w-12 sm:h-12" />
                            </div>
                            
                            <h2 className="text-red-600 text-2xl sm:text-4xl font-black uppercase mb-3 sm:mb-4 tracking-tighter">Violation Detected</h2>
                            
                            <div className="text-gray-600 space-y-4 sm:space-y-6">
                                <p className="text-sm sm:text-lg font-bold leading-relaxed border-y py-4 sm:py-6 border-red-50/50 italic px-2 sm:px-4">
                                    {violationMessage}
                                </p>
                            ...
                            <div className="mt-6 sm:mt-10 flex flex-col gap-6">
                                <button 
                                    ...
                                    className="px-6 sm:px-12 py-3.5 sm:py-5 bg-red-600 text-white font-black text-xs sm:text-base rounded-xl sm:rounded-2xl uppercase tracking-widest hover:bg-black hover:scale-105 active:scale-95 transition-all shadow-xl shadow-red-600/20"
                                >
                ...
                <div className="w-full max-w-4xl bg-white shadow-[0_20px_60px_rgba(0,0,0,0.05)] rounded-xl border border-gray-100 overflow-hidden">
                    <div className={`${
                        ...
                        'bg-[#0D47A1]'
                    } p-5 sm:p-8 text-center relative overflow-hidden`}>
                        ...
                        <h1 className="text-white text-xl sm:text-3xl font-black uppercase tracking-tight relative z-10">
                            {submissionType === 'VIOLATION' ? 'Terminated (Violation)' : 
                             submissionType === 'TIME_UP' ? 'Exam Time Over' : 
                             'Submission Report'}
                        </h1>
                        <p className="text-blue-100 text-[10px] sm:text-sm mt-1.5 sm:mt-2 font-medium opacity-80 tracking-wide uppercase">
    ...
    return (
        <div className={`flex flex-col h-screen ${isDarkMode ? 'bg-[#0f172a] text-slate-100' : 'bg-white text-gray-900'} font-sans overflow-hidden select-none transition-colors duration-300`}>
            ...
            <div className="bg-[#EF6C00] text-white px-4 md:px-6 py-2 flex justify-between items-center shadow-md z-10 min-h-[60px]">
                <h1 className="text-sm md:text-xl font-black truncate uppercase tracking-tight antialiased max-w-[40%] md:max-w-none">
                    {paperData.test_name}
                </h1>
                ...
                <div className="flex items-center gap-2 md:gap-3">
                    ...
                    <button 
                        ...
                        className="md:hidden px-3 py-1.5 rounded-full bg-white/10 hover:bg-white/20 transition-all text-white border border-white/20 flex items-center justify-center h-[36px] shadow-sm active:scale-95 font-black text-[10px] uppercase tracking-wider"
                    >
                        {mobileShowPalette ? "Question" : "Palette"}
                    </button>

                    <button 
                        onClick={toggleTheme}
                        className="p-1 rounded-full bg-white/10 hover:bg-white/20 transition-all text-white border border-white/20 flex items-center justify-center h-[36px] w-[36px] md:h-[40px] md:w-[40px] shadow-sm active:scale-95"
                        title={isDarkMode ? "Switch to Light Mode" : "Switch to Dark Mode"}
                    >
                        {isDarkMode ? <Sun className="w-4 h-4 md:w-5 md:h-5 text-yellow-300" /> : <Moon className="w-4 h-4 md:w-5 md:h-5 text-blue-100" />}
                    </button>
                    
                    <div className="bg-white px-3 md:px-8 py-1 md:py-2 shadow-lg flex items-center justify-center transition-colors min-w-[110px] md:min-w-[200px]">
                        <div className={`text-[#EF6C00] font-black text-xs md:text-xl tracking-tight transition-opacity
                    ...
                    {paperData.sections.map((section, idx) => {
                        ...
                        return (
                            <button
                                ...
                                className={`px-2 md:px-3 py-1 text-[9px] md:text-xs font-black rounded-[2px] transition-all uppercase shrink-0
                                ${activeSectionIdx === idx 
                                    ? 'bg-[#1565C0] text-white shadow-md border-b-2 border-white' 
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className={`flex-1 flex flex-col overflow-y-auto overflow-x-hidden ${isDarkMode ? 'bg-[#0f172a] border-slate-800' : 'bg-white border-gray-200'} border-r relative transition-colors ${mobileShowPalette ? 'hidden md:flex' : 'flex'}`}>
                    ...
                    <div className="flex-1 p-3 sm:p-6 md:p-8 relative overflow-x-hidden">
                        ...
                        <div className="flex flex-col mb-6 md:mb-8 relative z-10">
                            <div className={`flex items-center justify-between pb-3 md:pb-4 border-b mb-4 md:mb-6 ${isDarkMode ? 'border-slate-800' : 'border-gray-100'}`}>
                                <h3 className={`text-sm md:text-lg font-black uppercase tracking-tighter ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                                    Question No : {activeQuestionIdx + 1}
                                </h3>
                                <div className="flex items-center gap-1.5 md:gap-3 bg-white/5 p-0.5 md:p-1 rounded-[5px] pl-2 sm:pl-4 border border-transparent">
                                    <span className={`hidden sm:inline-block text-[10px] font-black uppercase tracking-[0.2em] ${isDarkMode ? 'text-slate-500' : 'text-gray-400'}`}>Focus Time</span>
                                    <div className={`px-2 md:px-4 py-1.5 rounded-[5px] font-black font-mono text-xs md:text-sm border shadow-sm transition-all
                            ...
                            >
                                ...
                                {currentQuestion.question_img && (
                                    <div className="my-4 sm:my-6">
                                        <img src={currentQuestion.question_img} alt="Question" className={`max-w-full h-auto border p-1.5 sm:p-2 ${isDarkMode ? 'bg-white/10 border-slate-700' : 'bg-white border-gray-200'} shadow-sm`} />
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="space-y-4 sm:space-y-6 ml-0 sm:ml-12 relative z-10">
                ...
                <div className={`w-full md:w-[380px] flex flex-col border-l transition-colors ${isDarkMode ? 'bg-[#0f172a] border-slate-800' : 'bg-white border-gray-200'} ${mobileShowPalette ? 'flex' : 'hidden md:flex'}`}>
                    {/* User Profile */}
                    ...
                    {/* Question Palette */}
                    <div className={`flex-1 p-4 sm:p-5 overflow-y-auto transition-colors ${isDarkMode ? 'bg-slate-900/30' : 'bg-blue-50/30'}`}>
