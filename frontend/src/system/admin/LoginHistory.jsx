        ...
        <div className={`p-10 rounded-[2.5rem] border shadow-xl ${isDarkMode ? 'bg-[#10141D] border-white/5' : 'bg-slate-100 border-slate-200 shadow-slate-200/50'}`}>
            ...
            <div className="space-y-4">
                {loginHistory.length > 0 ? loginHistory.map((log, i) => (
                    <div key={i} className={`flex items-center justify-between p-4 rounded-2xl border transition-all group ${isDarkMode ? 'bg-white/[0.03] border-white/5 hover:bg-white/[0.06]' : 'bg-slate-100 border-slate-200/50 hover:bg-slate-200'}`}>
